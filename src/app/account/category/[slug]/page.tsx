...
import { ArrowUp, ArrowDown, TrendingUp, Loader2, Info } from 'lucide-react';
...
export default function SingleCategoryPage() {
	...
	const fetchUserCategory = useCallback(async (identifier: string) => {
		...
		try {
			...
			if (result.status === 'success' && result.data && typeof result.data !== 'string') {
				...
				if (fetchedCategory.current_price_per_unit !== null && !isNaN(fetchedCategory.current_price_per_unit)) {
					setProductPrice(fetchedCategory.current_price_per_unit);
				} else {
					console.error('Fetched category price is invalid or missing:', fetchedCategory.current_price_per_unit);
					toast.error('Could not load product price from category data.');
				}
			} else {
	...
	const handleBuyNow = async () => {
		...
		setCurrentUserBalance(balance ?? undefined); // Update balance state
		...
		if (validationError) {
			...
			if (validationError === 'Insufficient balance.') {
				setIsModalOpen(true); // Open modal only for insufficient balance
			} else {
				toast.error(validationError); // Show other validation errors as toast
			}
		...
		console.log(`Sufficient balance (${formatNaira(balance)}). Proceeding with purchase of ${formatNaira(amount)}...`);
		// TODO: Add actual purchase API call here, using the validated 'amount'.
		toast.success(`Purchase of ${formatNaira(amount)} initiated! (Placeholder)`);
		// Optionally clear input after successful initiation: setAmountInput('');
