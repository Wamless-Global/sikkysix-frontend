...
export default function WithdrawReviewContent() {
	...
	const totalDebitCurrency = selectedNetwork?.feeCurrency || selectedAsset?.symbol;

	useEffect(() => {
		if (!assetId || !amountStr || !networkId || !recipientAddress || !selectedAsset || !selectedNetwork) {
			toast.error('Withdrawal details missing or invalid. Redirecting...');
			router.replace('/account/wallet/withdraw');
		}
	}, [assetId, amountStr, networkId, recipientAddress, selectedAsset, selectedNetwork, router]);

	if (!selectedAsset || !selectedNetwork || isNaN(amount) || !recipientAddress) {
		return (
	...
	const handleProceedToVerification = () => {
		...
		nProgress.start();
		router.push(`/account/wallet/withdraw/verify?assetId=${assetId}&amount=${amountStr}&networkId=${networkId}&address=${encodeURIComponent(recipientAddress)}`);
	};
		...
		<div className="max-w-2xl space-y-8">
			<Card className="bg-background border-0 shadow-none -mt-5">
				...
				<CardContent className="space-y-6 pt-6 px-0">
					<Card className="border-border">
						...
						<CardContent className="space-y-4 text-sm">
							...
							<div className="flex justify-between items-center py-3 border-b border-border">
								...
								<span className="font-medium text-foreground break-all">{decodeURIComponent(recipientAddress)}</span>
							</div>
							<div className="flex justify-between items-center py-3 border-b border-border">
					...
					<Button onClick={handleProceedToVerification} size="lg" variant="success" className="w-full flex items-center justify-center group" disabled={isProcessing}>
